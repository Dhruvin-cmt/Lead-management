...
import { addDeveloper } from "@/apis/dev-api";
import { zodResolver } from "@hookform/resolvers/zod";
import type z from "zod";
import { developerSchema } from "@/types/devloper.types";
import {
  User,
  Mail,
  Briefcase,
...
type IFormDev = z.infer<typeof developerSchema>;

export default function RegisterDeveloperCard({ regForm, fetchAgain }) {
  const {
    register,
    handleSubmit,
    watch,
  ...
  const selectedSkills = watch("techskills") || [];
  ...
  const onSubmit = async (data: IFormDev) => {
    setRootError(null);
    const payload = {
      ...data,
    ...
    try {
      const newData = developerSchema.safeParse(payload);
      await addDeveloper(newData.data);
      regForm(false);
      fetchAgain();
    } catch (error) {
      ...
      if (serverMessage) {
        // If the error message mentions "email", map it to the email field
        if (serverMessage.toLowerCase().includes("email")) {
          setError("email", { type: "server", message: serverMessage });
        } else {
          // Otherwise show it as a general error
          setRootError(serverMessage);
        }
      } else {
        setRootError(
          "Node interconnection failed. Please check your network or try again later."
        );
      }
    }
  };
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2.5">
                  ...
                  <Input
                    ...
                    className={cn(
                      "h-12 bg-slate-50/50 border-slate-200 focus:ring-indigo-500 focus:border-indigo-500 rounded-xl transition-all font-medium",
                      errors.expYearBeforeJoin &&
                        "border-rose-400 focus:ring-rose-500 focus:border-rose-500"
                    )}
                    {...register("expYearBeforeJoin", { valueAsNumber: true })}
                  />
                  {errors.expYearBeforeJoin && (
                    <p className="text-[10px] font-bold text-rose-500 uppercase tracking-tighter animate-in fade-in slide-in-from-top-1">
                      {errors.expYearBeforeJoin.message}
                    </p>
                  )}
                </div>
                <div className="space-y-2.5">
                  ...
                  <Input
                    ...
                    className={cn(
                      "h-12 bg-slate-50/50 border-slate-200 focus:ring-indigo-500 focus:border-indigo-500 rounded-xl transition-all font-medium",
                      errors.expMonthBeforeJoin &&
                        "border-rose-400 focus:ring-rose-500 focus:border-rose-500"
                    )}
                    {...register("expMonthBeforeJoin", { valueAsNumber: true })}
                  />
                  {errors.expMonthBeforeJoin && (
                    <p className="text-[10px] font-bold text-rose-500 uppercase tracking-tighter animate-in fade-in slide-in-from-top-1">
                      {errors.expMonthBeforeJoin.message}
                    </p>
                  )}
                </div>
              </div>
